...
sdb = [] #가공된 악성코드 DB가 저장된다(특정위치 검색법용)
...
def LoadVirusDB() :
    buf = DecodeKMD('virus.kmd')#악성 코드 패턴을 복호화한다.
...
def MakeVirusDB() :
    for pattern in VirusDB :
        ...
        scan_func = v[0] #악성코드 검사함수
        cure_func = v[1] #악성코드 치료함수

        if scan_func == 'ScanMD5' : #MD5해시를 이용한 검사인가?
            t.append(v[3]) #MD5해시를 저장한다.
            ...
            t.append(int(v[2])) #악성코드 진단 문자열의 위치를 저장
            t.append(v[3]) #악성코드 진단 문자열을 저장.
            t.append(v[4]) #악성코드 이름을 저장.
            sdb.append(t) #최종 sdb에 저장
